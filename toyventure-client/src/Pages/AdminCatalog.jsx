...
const AdminCatalog = () => {
  ...
  const [editingProduct, setEditingProduct] = useState({
    title: '', price: '', oldPrice: '', description: '', img: '', images: [], tag: '', category: '', countInStock: '',
    variants: [] 
  });
  ...
  useEffect(() => {
    const handleBeforeUnload = (e) => {
      if (isFormOpen && isCreatingNew) {
        const message = "You have unsaved changes. The blank product will remain in the database if you leave.";
        e.preventDefault();
  ...
  const handleCreateProduct = async () => {
    if (window.confirm('Create a new blank product template?')) {
      try {
        ...
        setEditingProduct({ 
          ...newProduct, 
          title: '', price: '', oldPrice: '', description: '', countInStock: '',
    ...
    try {
      const formattedProduct = {
        ...
        variants: editingProduct.variants.map(v => ({
            ...
            oldPrice: v.oldPrice === '' ? 0 : Number(v.oldPrice),
            countInStock: v.countInStock === '' ? 0 : Number(v.countInStock)
        }))
      };
  ...
  const handleFileUpload = async (e) => {
    ...
    if (files.length + currentCount > 7) { 
      alert(`Maximum 7 images allowed. You can only add ${7 - currentCount} more.`); 
      return; 
    }
    ...
    try {
      ...
      console.error(error); alert('Image upload failed.');
    } finally {
      setUploading(false);
    }
  };
  ...
  const handleAddVariant = () => {
    setEditingProduct({
      ...editingProduct,
      variants: [...editingProduct.variants, { color: '', size: '', description: '', price: editingProduct.price || '', oldPrice: '', countInStock: '', images: [] }]
    });
  };
  ...
  const handleRemoveVariant = (indexToRemove) => {
    setEditingProduct({
      ...editingProduct,
      variants: editingProduct.variants.filter((_, index) => index !== indexToRemove)
    });
  };
  ...
  const handleVariantFileUpload = async (e, variantIndex) => {
      ...
      if (files.length + currentCount > 4) { 
        alert(`Maximum 4 images per variant. You can only add ${4 - currentCount} more.`); 
        return; 
      }
      ...
      try {
        ...
        console.error(error); alert('Variant image upload failed.');
      } finally {
        setUploading(false);
      }
  };
            ...
            <div className="bg-white p-8 md:p-12 rounded-[3rem] border border-red-50 shadow-sm">
              <form onSubmit={handleUpdateProductSubmit} className="space-y-8">
                          ...
                          {editingProduct.images.map((img, idx) => (
                            <div key={idx} className="rounded-xl border bg-white p-1 shadow-sm aspect-square relative group">
                              ...
                              <button 
                                ...
                                className="absolute -top-2 -right-2 bg-red-500 text-white rounded-full w-6 h-6 flex items-center justify-center text-sm opacity-0 group-hover:opacity-100 transition-opacity hover:bg-red-700 z-10 shadow-md"
                                title="Remove Image"
                              >
                                &times;
                              </button>
                                   ...
                                   {variant.images.map((img, i) => (
                                     <div key={i} className="relative shrink-0 group/img">
                                       ...
                                       <button 
                                         ...
                                         className="absolute -top-2 -right-2 bg-red-500 text-white rounded-full w-6 h-6 flex items-center justify-center text-sm opacity-0 group-hover/img:opacity-100 transition-opacity hover:bg-red-700 shadow-md z-10"
                                         title="Remove Image"
                                       >
                                         &times;
                                       </button>
                ...
                <div className="pt-8 mt-8 border-t border-red-50 flex gap-4">
                  <button type="button" onClick={handleCancelForm} className="px-8 py-4 bg-red-50 text-red-950/60 font-black text-lg rounded-2xl hover:bg-red-100 transition-all">
                    Cancel
                  </button>
                ...
                <table className="w-full text-left border-collapse min-w-[1000px]">
                  <thead>
                    <tr className="bg-red-50/50 border-b border-red-50">
                      ...
                      <th className="p-5 text-xs font-black text-red-950/40 uppercase tracking-wider">Tags & Variants</th>
                  ...
                  <tbody>
                    {products?.map((product) => (
                      <tr key={product._id} className="hover:bg-red-50/30 transition-colors border-b border-red-50 group">
                        ...
                        <td className="p-5">
                          {/* --- UPDATED: Out of stock logic for admin table --- */}
                        ...
                        <td className="p-5">
                          ...
                          {product.variants && product.variants.length > 0 && (
                             <span className="bg-orange-50 text-orange-600 text-[10px] font-bold px-3 py-1 rounded-full border border-orange-100 mt-1.5 ml-2 inline-flex items-center gap-1">
                                <span className="material-symbols-outlined text-[12px]">style</span> {product.variants.length} Variants
                             </span>
                          )}
                        </td>
